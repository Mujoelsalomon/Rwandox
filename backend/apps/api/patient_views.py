...
from .dataset_history import dataset_patient_payloads
...
def patients_list_view(request):
    ...
    if not patients:
        patients = dataset_patient_payloads()
...
def patients_search_view(request):
    ...
    if not patients:
        dataset_patients = dataset_patient_payloads()
        if query:
            normalized = query.lower()
            dataset_patients = [
                patient for patient in dataset_patients
                if normalized in str(patient.get("hospital_id", "")).lower()
                or normalized in str(patient.get("surgery_type", "")).lower()
            ]
        patients = dataset_patients[:10]
