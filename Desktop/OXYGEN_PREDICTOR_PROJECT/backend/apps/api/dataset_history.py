...
def dataset_prediction_history_payloads(limit=250):
    ...
    with open(VIRTUAL_DATASET_PATH, newline="", encoding="utf-8") as dataset:
        for index, row in enumerate(csv.DictReader(dataset), start=1):
            oxygen_required = _bool(row.get("oxygen_required"))
            probability = _dataset_probability(row, oxygen_required)
            risk_level = _risk_level(probability)
            predictions.append({
                "id": f"dataset-{index}",
                "patient_id": row.get("hospital_id") or f"DATASET-{index:04d}",
                "age": _int(row.get("age"), 0),
                "sex": row.get("sex") or "Unknown",
                "surgery_type": row.get("surgery_type") or "Not recorded",
                "patient_disposition": row.get("ward") or _disposition(risk_level),
    ...
    with open(VIRTUAL_DATASET_PATH, newline="", encoding="utf-8") as dataset:
        for index, row in enumerate(csv.DictReader(dataset), start=1):
            ...
            patients.append({
                ...
                "hospital_id": row.get("hospital_id") or f"DATASET-{index:04d}",
                "name": f"Patient {row.get('hospital_id') or index}",
                "age": _int(row.get("age"), 0),
                "sex": row.get("sex") or "Unknown",
                "bmi": _float(row.get("bmi")),
                "smoking_history": _bool(row.get("smoking_history")),
                "comorbidities": row.get("comorbidities") or "",
                "baseline_spo2": _float(row.get("baseline_spo2")),
                "ward": row.get("ward") or _disposition(risk_level),
                "surgery_type": row.get("surgery_type") or "Not recorded",
                "risk_level": risk_level,
                "predicted_probability": probability,
                ...
                "latest_record": {
                    ...
                    "surgery_duration": _int(row.get("surgery_duration"), 0),
                    "blood_loss": row.get("blood_loss") or "",
...
def _dataset_probability(row, oxygen_required):
    ...
    baseline_spo2 = _float(row.get("baseline_spo2")) or 100
    postop_spo2 = _float(row.get("postop_spo2")) or 100
    duration = _int(row.get("surgery_duration"), 0)
    respiratory_rate = _int(row.get("respiratory_rate"), 0)
    ...
    if respiratory_rate >= 24:
        score += 6
    if _bool(row.get("opioid_use")):
        score += 5
    if _bool(row.get("residual_effects")):
        score += 5
    if str(row.get("urgency") or "").lower() == "emergency":
        score += 6
...
def _dataset_contributing_factors(row):
    ...
    if (_float(row.get("baseline_spo2")) or 100) < 94:
        factors.append("Low baseline SpO2")
    if _int(row.get("surgery_duration"), 0) >= 180:
